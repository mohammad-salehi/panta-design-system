...
// کامپوننت داخلی که از useTheme استفاده می‌کند
const AppShellInner: React.FC<AppShellProps> = ({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent | TouchEvent) => {
      ...
      if (
        (asideRef.current && asideRef.current.contains(target)) ||
        (mobileSidebarRef.current && mobileSidebarRef.current.contains(target))
      ) return;
  ...
  const UserCard = () => (
    <div className="mt-3 rounded-2xl border border-boxBorderColor bg-boxColor/70 p-3 lux-icon">
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-xl border-boxBorderColor dark:border-boxBorderColor-dark bg-white/70 dark:bg-bgColor-dark/60 transition flex items-center justify-center text-titleText dark:text-titleText-dark lux-icon">
        ...
        <div className="min-w-0 flex-1" dir="ltr">
          <div className="text-sm font-semibold truncate lux-text">{userFullName}</div>
          <div className="text-[11px] text-gray-500 truncate lux-text">{userRole}</div>
        </div>
        {onChangePassword && (
          <button
  ...
  const NavItems = () => (
    <nav className="px-3 pt-4 space-y-2 overflow-y-auto flex-1 min-h-0">
      {navItems.map((item) => {
        if (item.access && item.access !== userRole) return null;
        const isActive = currentPath === item.link;
        return (
          <a href={item.link} key={item.label} style={{ display: "block", textDecoration: "none", color: "inherit" }}>
            <button
              type="button"
              className={clsx(
            ...
            >
              <div className="flex items-center gap-4 min-w-0">
                ...
                <span className="text-sm font-medium text-right leading-5 min-w-0 break-words lux-text" dir="rtl">
                  {item.label}
                </span>
              </div>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                className={clsx("transition self-center shrink-0", isActive ? "opacity-100" : "opacity-40")}>
  ...
  const ThemeToggle = ({ className: cls }: { className?: string }) => (
    <button className={clsx("flex items-center justify-center transition lux-btn", cls)} onClick={toggleTheme}>
      {isDarkMode ? (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
        ...
        <div className="p-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {brand ?? <div className="font-bold text-titleText">لوگو</div>}
            </div>
  ...
  return (
    <div>
      ...
      >
        {/* Desktop Header */}
        <header className="hidden lg:flex mx-8 h-18 items-center justify-between shadow-sm px-6 lux-panel rounded-t-none">
          <div className="flex items-center gap-5">
            <span className="font-bold text-titleText dark:text-titleText-dark">
              {headerTitle || title}
            </span>
          </div>
        ...
        {/* Page Content */}
        <main className="mx-8 mt-5 pt-14 lg:pt-0">
          <ToastProvider>
            {children}
          </ToastProvider>
        </main>
